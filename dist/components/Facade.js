...
const WEBHOOK_URL = 'https://n8n-n8n.n1n956.easypanel.host/webhook-test/Fimm-Fachada-da-Casa';
function Facade() {
    ...
    const uploadImage = async () => {
        ...
        if (!title.trim()) {
            toast({
                title: "Error",
                description: "Please enter a facade title",
                variant: "destructive",
            });
            return;
        }
        if (!location.trim()) {
            toast({
                title: "Error",
                description: "Please enter the facade location",
                variant: "destructive",
            });
            return;
        }
        try {
            ...
            const fileName = `fachada_${timestamp}.jpg`;
            ...
            toast({
                title: "Success!",
                description: `Facade image uploaded to Supabase Storage as ${fileName} and processing started`,
                duration: 5000,
            });
    ...
    return (<card_1.Card className="p-6 max-w-2xl mx-auto">
      <div className="space-y-4">
        {!imgSrc ? (<div className="relative rounded-lg overflow-hidden bg-black">
            ...
            <img src={imgSrc} alt="Captured Facade" className="w-full rounded-lg"/>
            
            <div className="space-y-4">
              <div className="space-y-2">
                <label_1.Label htmlFor="title">Facade Title</label_1.Label>
                <input_1.Input id="title" placeholder="Enter facade title" value={title} onChange={(e) => setTitle(e.target.value)} disabled={isUploading}/>
              </div>
              
              <div className="space-y-2">
                <label_1.Label htmlFor="location">Facade Location</label_1.Label>
                <input_1.Input id="location" placeholder="Enter facade location" value={location} onChange={(e) => setLocation(e.target.value)} disabled={isUploading}/>
              </div>
            </div>
